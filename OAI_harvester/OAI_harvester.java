...
public final class OAI_harvester
{
    ...
    public static String PATH_TO_TMP = sep+"usr"+sep+"local"+sep+"log"+sep+"harvester";
    ...
    public static void main( String[] args ) 
    {
        ...
        System.out.println("OAI_harvester version : v.0.0.4 (xsltproc) \n");
        parse_config();
        ...
        if ((String)((HashMap)config.get(harvester_name)).get("oai_from") != null) {
            from=(String)((HashMap)config.get(harvester_name)).get("oai_from");
    ...
    public static boolean transform_data(String harvester_name) throws Exception
    {
        ...
        /*  Xallan's does not allow me to use document() statement in xslt, falling back to good old xsltproc */

        try {
            BufferedWriter out = new BufferedWriter(new OutputStreamWriter(new FileOutputStream("/tmp/xml_in_"+harvester_name+".xml"), "UTF-8")); 
            StringReader r = new StringReader(xml_in.toString());
            int c;
            while ((c = r.read()) != -1) {
                out.write((char)c);
            }
        ...
        try {
            String[] command = {"/usr/bin/xsltproc", path+sep+selected_harvester.get("solr_pre_xslt"), "/tmp/xml_in_"+harvester_name+".xml"};
    ...
    private static boolean get_oai_data(String name)
    {
        ...
        System.out.println("Open conn : " +url.toString());

        try {
            URLConnection conn = url.openConnection();
            conn.setDoInput(true);
            conn.setUseCaches(false);
            //DataInputStream dis = new InputStreamReader(conn.getInputStream(), "UTF-8"); 
        ...
        try {
            ...
            System.out.println("I can haz bufferd readertz !");
        ...
        */
        ...
        System.out.println("Done fetching : " + url.toString());
    ...
    private static String find_next_token()
    {
        ...
        try {
            ...
            XPathExpression expr = xpath.compile("//resumptionToken/text()");
