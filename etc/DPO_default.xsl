...
<xsl:template match="//didl:DIDL">
    <xsl:apply-templates name="didl:Item"/>
</xsl:template>
...
<xsl:template match="didl:Item">
    <xsl:for-each select="didl:Item">
        ...
        <xsl:variable name="docid" select="@*|node()"/>
        ...
        <xsl:text disable-output-escaping="yes">"&gt;</xsl:text>
            <xsl:value-of select="document(concat('http://resolvero.kb.nl/resolve?urn=', $docid, ':ocr'))"/>
        <xsl:text disable-output-escaping="yes">&lt;/field&gt;</xsl:text>
        <xsl:text disable-output-escaping="yes">&lt;/add&gt;</xsl:text>
        <xsl:text>
        </xsl:text>
    ...
    <xsl:choose>
    <xsl:text disable-output-escaping="yes"> &lt;add&gt;</xsl:text>
        <xsl:when test="string-length(substring-after(@*|node(), ':mpeg21:')) = 4">
                <xsl:apply-templates name="didl:Resource" select="*"/>
        </xsl:when>
    </xsl:choose>
    <xsl:text disable-output-escaping="yes"> &lt;/add&gt;</xsl:text>
    -->
...
<xsl:template match="didl:Resource/*">
    <xsl:text disable-output-escaping="yes"> &lt;doc&gt;</xsl:text>
        <xsl:for-each select="//srw_dc:dcx/*">
        <xsl:text disable-output-escaping="yes"> &lt;add&gt;</xsl:text>
            <xsl:for-each select="//srw_dc:dc/*">
                <xsl:text disable-output-escaping="yes">&lt;field name="</xsl:text>
                <xsl:value-of select="substring-after(name(), ':')"/>
